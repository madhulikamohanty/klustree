...
public class RTEDCommandLine {
	 ...
	 * @param args
	 */
	public double runCommandLine(String[] args) {
	...
	public static ArrayList<String> Isomorphism(ArrayList<String> alltrees) {
		int treeCount = alltrees.size();
		ArrayList<String> cluster = new ArrayList<String>();		
		for(int i = 0;i<treeCount;i++){
			String x=alltrees.get(i);
			String out ="";
			if(!x.equals("DONE")){
				out=(i+1)+"";

				for(int j=(i+1);j<treeCount;j++){
					String y = alltrees.get(j);
					if(x.equals(y)){
						out += "-"+(j+1);
					}
				}
				...
				for(int K=0; K<ar.length; K++){
					alltrees.set(Integer.parseInt(ar[K])-1,"DONE");
				}
				if(!out.equals(""))
					cluster.add(out);
			}
	...
	public static double[][] generateEditSimilarity(ArrayList<String> trees) throws IOException {

		int treeCount = trees.size();
		System.out.println("TreeCount : " + treeCount);
		double[][] editSimilarity = new double[treeCount][treeCount];

		RTEDCommandLine rtedCL = new RTEDCommandLine();
		String[] args1 = new String[4];
		args1[0] = "-t";
		for(int i=0; i< treeCount; i++){
			args1[1]= trees.get(i).replaceAll("[:]", "");
			for(int j=i; j<treeCount; j++){
				args1[2]= trees.get(j).replaceAll("[:]", "");
				args1[3]= "-k";
				double ted = rtedCL.runCommandLine(args1);
				int nodeCountinT1 = args1[1].replaceAll("[{}]", "DELIMITER").split("DELIMITER").length;
				int nodeCountinT2 = args1[2].replaceAll("[{}]", "DELIMITER").split("DELIMITER").length;
				editSimilarity[i][j] = 1 - (ted/(nodeCountinT1 + nodeCountinT2));
				editSimilarity[j][i] = editSimilarity[i][j];
			}
		}

		double[][] columnSimilarity = new double[treeCount][treeCount];
		for(int i=0; i<treeCount; i++){
			for(int j=i; j<treeCount; j++){
				double sum=0;
				for(int k=0; k<treeCount; k++){

					sum = sum + Math.abs(editSimilarity[k][i] - editSimilarity[k][j]);
				}

				columnSimilarity[i][j] = 1- (sum/treeCount);
				columnSimilarity[j][i] = columnSimilarity[i][j];
			}
		}
