...
public class IsoCluster extends HttpServlet 
{
			...
			throws IOException,ServletException 
	{
		...
		if(req.getParameter("q_type").equals("browser")){
			...
			try {
				...
				if (ds==null) {
					wr.printIsoWaitScreen(req.getQueryString());
					return;
				}
				...
				ArrayList<String> clusters = util.RTEDCommandLine.Isomorphism(allTrees);
				...
				for(int j=0;j<clusters.size();j++){
					...
					try{
						...
						String maxFile1=Config.statsDirectory +Config.isomaxMinDirectory+query.replace(" ", "_").replace("'", "")+
								"_max"+(j+1)+"-1.json";
						String maxFile2=Config.statsDirectory +Config.isomaxMinDirectory+query.replace(" ", "_").replace("'", "")+
								"_max"+(j+1)+"-2.json";
						...
						while(maxIter.hasNext()){
							...
							FileEdit.writeToFile(writeFile, "]");
							File mff = new File(getServletContext().getRealPath("/")+"/stats/"+Config.isomaxMinDirectory+query.replace(" ", "_").replace("'", "")+
									"_max"+(j+1)+"-"+max_i+".json");
						...
						String representativeFile=Config.statsDirectory + "/"+Config.isorepDirectory+query.replace(" ", "_").replace("'", "")+
								"_rep"+(j+1)+".json";
						...
						File repff = new File(getServletContext().getRealPath("/")+"/stats/"+Config.isorepDirectory+query.replace(" ", "_").replace("'", "")+
								"_rep"+(j+1)+".json");
						if(repff.exists()){
							repff.delete();
						}
				...
				for(i =0; i< clusters.size(); i++){
					...
					String jsonRepFile = Config.host+"/TreeEdit/MyAns.html?answerFile=TreeEdit/stats/"+Config.isorepDirectory+
				...
				for(i =0; i< clusters.size(); i++){
					...
					String jsonFile =Config.host+"TreeEdit/MyAns.html?answerFile=TreeEdit/stats/Iso_clusterFiles/"+
							query.replace(" ", "_").replace("'", "")+"_"+(i+1)+".json";
					pw.println("<iframe width=\"100%\" height=\"100%\" src="+jsonFile + "></iframe>");

					String filename = getServletContext().getRealPath("/")+"/stats/Iso_clusterFiles/"+query.replace(" ", "_").replace("'", "") +
							"_" +(i+1) +".json";
					...
					File f = new File(Config.statsDirectory+"Iso_clusterFiles/" +
			...
			String clusterCountFile = Config.statsDirectory + "/clusterCount-iso.txt";
				...
				try {
					...
					for(int j=0;j<clusters.size();j++){
						...
						try{
							...
							String maxFile1=Config.statsDirectory +Config.isomaxMinDirectory+queryString.replace(" ", "_").replace("'", "")+
									"_max"+(j+1)+"-1.json";
							String maxFile2=Config.statsDirectory +Config.isomaxMinDirectory+queryString.replace(" ", "_").replace("'", "")+
									"_max"+(j+1)+"-2.json";
							...
							while(maxIter.hasNext()){
								...
								FileEdit.eraseLast(writeFile);
								FileEdit.writeToFile(writeFile, "]");
								File mff = new File(getServletContext().getRealPath("/")+"/stats/"+Config.isomaxMinDirectory+queryString.replace(" ", "_").replace("'", "")+
										"_max"+(j+1)+"-"+max_i+".json");
							...
							//Writing Representative file.
							String representativeFile=Config.statsDirectory + "/"+Config.isorepDirectory+queryString.replace(" ", "_").replace("'", "")+
									"_rep"+(j+1)+".json";
							File rf = new File(representativeFile);
							if(rf.exists()){
								rf.delete();
							...
							File repff = new File(getServletContext().getRealPath("/")+"/stats/"+Config.isorepDirectory+queryString.replace(" ", "_").replace("'", "")+
									"_rep"+(j+1)+".json");
							if(repff.exists()){
								repff.delete();
							}
					...
					for(i =0; i< clusters.size(); i++){
						...
						pw.println("<iframe width=\"100%\" height=\"100%\" src="+jsonFile + "></iframe>");
						pw.println("</div>");
						String filename = getServletContext().getRealPath("/")+"/stats/"+"Iso_clusterFiles/"+queryString.replace(" ", "_").replace("'", "") +
								"_" +(i+1) +".json";
